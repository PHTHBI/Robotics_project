...
try
    ...
    disp('Moving robot to test position...');
    testPosition = [150, 160, 100, 41]; % Angles in degrees within your robot's limits
    DXL_IDS = [1, 2, 3, 4];
    robot.robotMove(port_num, testPosition, DXL_IDS);
    disp('Robot moved to test position.');

    % Capture an image of the chessboard
    %disp('Capturing chessboard image...');
    %cameraCalib.captureChess(3);
    %disp(['Image saved to ', chessboard_image_path]);

    % Perform camera calibration using the captured image
    %disp('Calibrating camera...');
    %cameraCalib.calibrate(true);
    ...
    gridCorners = boardDetector.detectFourIntersections();
    disp('Grid Corners:');
    disp(gridCorners);

    % Detect grid cell centers
    disp('Detecting grid cell centers...');
    cellCenters = boardDetector.findGridCellCenters(gridCorners);
    disp('Cell Centers:');
    disp(cellCenters);
    ...
    imshow(img);
    hold on;
    plot(gridCorners(:, 1), gridCorners(:, 2), 'ro', 'MarkerFaceColor', 'r'); % Red dots for grid corners
    plot(cellCenters(:, 1), cellCenters(:, 2), 'bo', 'MarkerFaceColor', 'b'); % Blue dots for cell centers
    hold off;


     %% Setup the Tic Tac Toe board and cell centers
    ticTacToeGame = ticTacToeGame.setupBoard();
