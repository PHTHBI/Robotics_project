function crosses = find_crosses(image)
    ...
    smoothed_image = imgaussfilt(image, 1.5);  % Slightly less smoothing for better detail retention
    ...
    peaks = houghpeaks(H, 40, 'Threshold', ceil(0.2 * max(H(:)))); % More sensitive to weaker lines
    ...
    diagonal_lines = [];
    min_length = 10;  % Reduced minimum line length to detect smaller features
    angle_tolerance = 15;  % Broadened angle tolerance for increased sensitivity
    min_endpoint_separation = 5;  % Minimum distance between endpoints for diagonal lines
    ...
    if ~isempty(detected_crosses)
        ...
        while ~isempty(detected_crosses)
            ...
            close_indices = find(distances <= 10);
