%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%       Robot Movement & Camera Testing - 4
...
% Capture initial image of the tic-tac-toe board
img = snapshot(cam);
imshow(img);
title('Captured Tic-Tac-Toe Board');
drawnow; % Update the display immediately

% Image processing to identify tic-tac-toe board
% Convert to grayscale and apply thresholding to make lines clearer
gray_img = rgb2gray(img);
bw_img = imbinarize(gray_img, 'adaptive', 'ForegroundPolarity', 'dark', 'Sensitivity', 0.4);
imshow(bw_img);
title('Processed Image of Tic-Tac-Toe Board');
drawnow;

% Detect grid lines (using Hough transform or morphological operations)
% and define grid cell coordinates for marking positions

% For simplicity, hardcode the center cell's location
% Adjust coordinates based on calibration (experiment with your setup)
%center_cell_x = 320; % X coordinate of the center cell
%center_cell_y = 240; % Y coordinate of the center cell
