function board = boardMatrixFill(square_centers, cross_centers, circle_centers)
    ...
    board = zeros(3, 3);
    acceptable_error = 40; % Acceptable error in position matching
    ...
    for i = 1:9
        ...
        if ~isempty(cross_centers)
            ...
            if any(distances_to_crosses <= acceptable_error)
                x_detected = true;
                board(row, col) = 1; % Tentatively assign X to the square
            end
        end
        ...
        if ~isempty(circle_centers)
            ...
            if any(distances_to_circles <= acceptable_error)
                o_detected = true;
                board(row, col) = -1; % Assign O to the square, overwriting X if necessary
            end
        end

        % Resolve conflicts: if both X and O are detected, prioritize O
        if x_detected && o_detected
            board(row, col) = -1;
        end
